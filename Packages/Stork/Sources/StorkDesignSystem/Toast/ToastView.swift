...
//  StorkDesignSystem
//
//  Top-of-screen toast rendering for `ToastManager`. Apply `.toastContainer()` once near
//  the app root; it reads the manager from the environment.
//
...
public struct ToastView: View {
    ...
    public var body: some View {
        ToastContent(toast: toast, onDismiss: onDismiss)
...
struct ToastContainerModifier: ViewModifier {
    ...
    /// iPad anchors toasts to the top-leading corner so they don't crowd the split-view
    /// title area.
...
public extension View {
    /// Overlays the current toast (from `ToastManager` in the environment) at the top.
