...
struct OnboardingLocationPage: View {
    ...
    private var statusConfig: (icon: String, color: Color, title: String, description: String) {
        ...
        case .authorizedAlways, .authorizedWhenInUse:
            ("checkmark.circle.fill", .green, "Location Enabled", "We'll use your location for local weather.")
        case .denied, .restricted:
            ("location.slash.fill", .orange, "Location Disabled", "Enable location in Settings to get weather data.")
        case .notDetermined:
            ("location.circle.fill", .storkBlue, "Enable Location", "Allow location access to see local weather on your dashboard screen on iPhone.")
        @unknown default:
            ("questionmark.circle.fill", .secondary, "Unknown Status", "We couldn't determine your location settings.")
        }
    }
