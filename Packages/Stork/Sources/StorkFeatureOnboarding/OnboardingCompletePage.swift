        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ForEach(Array(features.enumerated()), id: \.offset) { index, feature in
                    HStack(spacing: 14) {
                        Image(systemName: feature.icon).font(.title3).foregroundStyle(feature.color).frame(width: 28).accessibilityHidden(true)
                        Text(feature.title).font(.body)
                        Spacer()
                        Image(systemName: "checkmark").font(.subheadline.weight(.semibold)).foregroundStyle(.green).accessibilityHidden(true)
                    }
                    ...
                    if index < features.count - 1 { Divider().padding(.leading, 56) }
                }
            }
